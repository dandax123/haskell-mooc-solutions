...
-- a picture returns a color, given two cordniates, so we turn that colorToHex
...
instance Transform Flip where
  apply FlipX (Picture f ) = Picture (f.flipxy)
  apply FlipXY t = flipXY t
  apply FlipY (Picture f ) = Picture (f.flipxy')
------------------------------------------------------------------------------
...
avgColors :: Color -> Color -> Color -> Color -> Color -> Color
avgColors (Color v n m) (Color a b c) (Color d e f ) (Color g h i)  (Color j k l) = Color (div (a+d+g+j+v) 5) (div (b+e+h+k+n) 5) (div (c+f+i+l+m) 5)
...
blur (Picture f) = Picture k
    where k (Coord x y) = avgColors  (f (Coord x y)) ( f (Coord (x-1) y)) (f (Coord (x+1) y)) (f (Coord x (y-1))) (f (Coord x (y+1)))

data BlurMany = BlurMany Int
  deriving Show

instance  Transform BlurMany where
  apply (BlurMany n) f | n > 0 =  apply (BlurMany (n-1)) (blur f)
    | otherwise  = f
------------------------------------------------------------------------------
