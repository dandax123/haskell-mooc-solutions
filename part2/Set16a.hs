...
inputInOutput input output = forAll (elements input) f'
  where
    f' x = x `elem` map fst output
...
outputInInput input output = forAll (elements output) f'
  where
    f' (x, n) = n === length (filter (== x) input)
...
frequenciesProp freq (NonEmpty a) = conjoin [sumIsLength a b, inputInOutput a b, outputInInput a b] where b = freq a
