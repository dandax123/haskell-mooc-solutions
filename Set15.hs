...
statements x y = liftA2 combine x y ++ liftA2 combine' x y
  where
    combine x y = x ++ " is " ++ y
    combine' x y = x ++ " is not " ++ y
...
--
data Op = Negate | Double

parseOpstring :: String -> Maybe Op
parseOpstring "negate" = pure Negate
parseOpstring "double" = pure Double
parseOpstring _ = Nothing
...
calculator a b = liftA2 runOps (parseOpstring a) (readMaybe b)
...
validateDiv a b = fmap (div a) checkedB
  where
    checkedB = check (b /= 0) "Division by zero!" b
...
twoPersons ::
  ...
  f Bool ->
  f [Person]
twoPersons name1 age1 employed1 name2 age2 employed2 = liftA2 (++) (fmap (: []) (liftA3 Person name1 age1 employed1)) (fmap (: []) (liftA3 Person name2 age2 employed2))
...
instance Applicative Priced where
  pure a = Priced 0 a
  liftA2 f (Priced y b) (Priced x c) = Priced (x + y) (f b c)
...
myFmap op = myLiftA2 d (myPure Nothing)
  where
    d c b = op b

-- myFmapHelper :: (a -> b) -> (a -> b -> b)
-- myFmapHelper op =
------------------------------------------------------------------------------
...
instance (Applicative f, Applicative g) => Applicative (Both f g) where
  ...
  liftA2 op c@(Both a) d@(Both b) = Both (liftA2 d a b)
    where
      d y z = liftA2 op y z
